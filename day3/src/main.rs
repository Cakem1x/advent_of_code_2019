...
fn main() {
    ...
    let closest_intersection = get_closest_intersection(&wire1, &wire2).expect("No interesction found!");
    println!("Closest intersection point is {:?} with distance {}", closest_intersection.0, closest_intersection.1);
}
    ...
    for line1 in &wire1[..] {
        for line2 in &wire2[..] {
            match line1.intersect(line2) {
                Some(intersection_point) => if intersection_point != origin {intersection_points.push(intersection_point)} else {()},
                None => (),
            }
        }
    }
